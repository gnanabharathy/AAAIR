import os, sys, json, re, csv, urllib.request, urllib.error, webbrowser, time, socket
...
def regex_extract_last_updated(html):
    # Matches: <h5 ...>Last Revised: September 2009</h5>
    # or plain text: Last Revised: September 2009
    patterns = [
    ...
    for p in patterns:
        ...
        if m:
            val = m.group(1).strip()
            if val.lower() not in ('na', 'n/a', '', 'not applicable'):
                return val
    return None
...
# NIM helpers
# ---------------------------------------------------------------------------
_last_nim_call_time = [0]

def nim_chat(messages, max_tokens=4096, max_retries=5):
    if not NIM_API_KEY:
        raise RuntimeError("NIM_API_KEY not set in .env")
    
    elapsed = time.time() - _last_nim_call_time[0]
    if elapsed < 1.8:
        time.sleep(1.8 - elapsed)
    _last_nim_call_time[0] = time.time()
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if e.code == 429:
                ...
                print(f"HTTP {e.code} Gateway/Server error. Waiting {wait_time} seconds before retrying...")
                time.sleep(wait_time)
            else:
                raise
            ...
            if attempt < max_retries - 1:
                wait_time = 15 * (attempt + 1)
                print(f"Request timed out ({e}). Retrying in {wait_time}s "
                      f"(attempt {attempt + 1}/{max_retries})...")
                time.sleep(wait_time)
            else:
                raise

    raise RuntimeError("NIM request failed after multiple retries (rate limiting or timeouts).")
...
def nim_extract_card_info(html, ds_id, url):
    ...
    reply = nim_chat([
        ...
        {"role": "user",   "content": (
            ...
            "  name     – short descriptive name, MUST include the specific year range and file type\n"
            "             e.g. 'NHANES Demographics 2015-16 (DEMO_I)', 'NHANES Cholesterol 2017-18 (TCHOL_J)'\n"
...
def run_single(url, ds_id, review=True, all_results=None):
    print(f"\n{'='*60}")
    ...
    try:
        ...
        # Step 3: Last updated
        print("[3/4] Last updated...")
        # First try page content (Last Revised date — most accurate)
        last_updated = regex_extract_last_updated(html)
        # If NA or not found, fall back to HTTP header
        if not last_updated:
        ...
        print(f"      {last_updated or 'not found'}")
        ...
        if len(variables) != expected_count:
            print(f"      WARNING: count changed ({len(variables)} vs {expected_count})")
        ...
        result = {
            ...
            "desc":           card.get("desc", ""),
            "source":         "NHANES" if "nhanes" in ds_id.lower() else card.get("source", ""),
        ...
        if review:
            ...
            build_review_page(ds_id, url, variables, last_updated,
                              expected_count, review_path)
        ...
        update_data_js(all_results, card, "data.js")
        print(f"Saved. ({len(all_results)} datasets total)")
...
def main():
    ...
    # Single URL mode: python3 extractor.py <url> [id]
    if len(sys.argv) >= 2 and sys.argv[1].startswith("http"):
        url   = sys.argv[1].strip()
        ds_id = (sys.argv[2].strip() if len(sys.argv) > 2
                 else url.split("/")[-1].replace(".htm","").replace(".html",""))
        run_single(url, ds_id, review=True)
        return

    # Batch mode: python3 extractor.py --batch [csv_file]
    if len(sys.argv) >= 2 and sys.argv[1] == "--batch":
        csv_file = sys.argv[2] if len(sys.argv) >= 3 else "datasets_nhanes_full.csv"
    else:
        print("Usage:")
        print("  Single: python3 extractor.py <url> [id]")
        print("  Batch:  python3 extractor.py --batch [csv_file]")
        sys.exit(1)
    ...
    with open(csv_file) as f:
        datasets = [(r["id"], r["url"])
                    ...
                    if r.get("id","").strip() and r.get("url","").strip()]

    print(f"Batch mode: {len(datasets)} datasets from {csv_file}")
    ...
    if os.path.exists(schema_path):
        with open(schema_path) as f:
            all_results = json.load(f)
        print(f"Loaded existing schema.json ({len(all_results)} already done)")

    done  = len([d for d in datasets if d[0] in all_results])
    total = len(datasets)
    print(f"Progress: {done}/{total} done, {total-done} remaining\n")

    for ds_id, url in datasets:
        if ds_id in all_results and all_results[ds_id].get('source') and all_results[ds_id].get('subtypes'):
            print(f"[skip] {ds_id}")
            continue
        run_single(url, ds_id, review=False, all_results=all_results)

    print(f"\nAll done. schema.json has {len(all_results)} datasets.")
